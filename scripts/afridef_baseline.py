"""
AfriDef BASELINE — KSM 2026 Ablation
Pure BCE, no WGAN-GP augmentation, no TRADES adversarial training.
Same architecture, same seeds, same calibration + Stackelberg as AfriDef v2.

Purpose: establish the contribution of WGAN-GP + TRADES by comparison.
"""
...
VAL_FRAC     = 0.10
TEST_FRAC    = 0.20
RESULTS_DIR  = "/kaggle/working/afridef_results_baseline"
...
# ── Graph builder ───────────────────────────────────────────────────────────
def build_graph(df, n_train, n_val, n_test, device):
    ...
    print(f"  Graph: {N:,} nodes | {n:,} edges | "
          f"train fraud={int(y[tm].sum())} val={int(y[vm].sum())} test={int(y[xm].sum())}")
...
# ── Plain BCE loss (no TRADES) ─────────────────────────────────────────────
def bce_loss(model, x, y, ei, ea, mask, pos_weight):
    model.train()
    logits = model(x, ei, ea, mask=mask)
    return F.binary_cross_entropy_with_logits(
        logits, y[mask], pos_weight=pos_weight.to(x.device))
...
def stackelberg_threshold(model, data, eps=0.05, outer=3, flip=False):
    ...
    for _ in range(outer):
        ...
        model.eval()
        with torch.no_grad():
            logits = model(x_adv, data.edge_index, data.edge_attr, mask=data.val_mask)
            probs  = torch.sigmoid(logits).cpu().numpy()
        if flip:
            probs = 1.0 - probs
...
def run_seed(seed, df_orig, device):
    ...
    # No WGAN-GP: use original data directly
    df_sorted = df_orig.sort_values("step").reset_index(drop=True)
    data = build_graph(df_sorted, n_train, n_val, n_test, device)
    ...
    print(f"  Training {N_EPOCHS} epochs (plain BCE, no TRADES, no WGAN-GP) ...")
    best_val, best_state = 0.0, None
    for ep in range(1, N_EPOCHS + 1):
        model.train()
        opt.zero_grad()
        loss = bce_loss(model, data.x, data.y, data.edge_index, data.edge_attr,
                        data.train_mask, pw)
        loss.backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
        opt.step()
        if ep % 10 == 0:
            auroc, ap, f1 = evaluate(model, data, data.val_mask, flip=False)
    ...
    # Same calibration logic as v2 (fair comparison)
    flip = False
    if best_val < 0.5:
        flip = True
        calibrated_auroc = 1.0 - best_val
        print(f"  [CALIBRATION] val AUROC={best_val:.4f} < 0.5 — "
              f"flipping (calibrated val AUROC={calibrated_auroc:.4f})")
    ...
    if n_pos_test == 0:
        print("  [WARN] No fraud in test set — reporting NaN")
...
if __name__ == "__main__":
    ...
    print(f"AfriDef BASELINE | Plain BCE | No WGAN-GP | No TRADES | SEEDS={SEEDS}")
    ...
    print(f"\n{'='*60}")
    print("FINAL SUMMARY — AfriDef BASELINE (plain GNN + BCE, no augmentation)")
    print("=" * 60)
